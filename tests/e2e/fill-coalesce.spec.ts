...
test('Fill Default and Coalesce process a real CSV example', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('button', { name: 'Clean' }).click();
  await page.locator('input[type="file"]').first().setInputFiles('public/examples/fill-coalesce.csv');
  await page.getByRole('button', { name: 'Continue' }).click();
  await page.locator('label.settings-load input[type="file"]').setInputFiles('public/examples/fill-coalesce-settings.json');
  await page.getByRole('button', { name: 'Run preview' }).click();
  await expect(page.getByRole('cell', { name: 'US', exact: true }).first()).toBeVisible();
